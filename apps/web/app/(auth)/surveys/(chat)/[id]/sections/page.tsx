...
const Page = ({ params }: Props) => {
  ...
  // 新規調査作成の内容に基づいてセクションを自動生成しDBに保存（重複防止）
  const autoSyncedRef = useRef<number | null>(null);
  useEffect(() => {
    const run = async () => {
      if (!survey || !sections) return;
      if (sections.length !== 0) return;
      if (autoSyncSections.isPending) return;
      if (autoSyncedRef.current === surveyId) return;
      ...
      try {
        autoSyncedRef.current = surveyId;
  ...
  // 質問が無ければダミー設問を生成（重複防止）
  const seedMutation = api.question.seedForSurvey.useMutation({});
  const seededRef = useRef<number | null>(null);
  useEffect(() => {
    if (!questionsData || questionsData.length === 0) return;
    const total = questionsData.reduce((sum: number, s: any) => sum + (s.questions?.length || 0), 0);
    if (total !== 0) return;
    if (seedMutation.isPending) return;
    if (seededRef.current === surveyId) return;

    seededRef.current = surveyId;
  ...
  const generateMainSections = (surveyData: any) => {
    ...
    // 利用状況・体験
    if (surveyData.analysisCondition && 
    ...
    return sections.length > 0 ? sections : ["利用状況・体験", "今後の利用意向"];
  };
